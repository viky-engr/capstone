...
def form():
    ...
    if request.method == 'POST':
        ...
        if sex =='M' or sex=='F' or sex=='X':  #check for valid sex input - can add checks to other inputs later.
            ...
            os.rename(ECG_rest.filename, '/pre_exercise_ecg.txt') #change the filename and move to Website folder
            os.rename(ECG_exercise.filename, '/post_exercise_ecg.txt')
...
def result():
    ...
    preexercise_path = our_path + '/pre_exercise_ecg.txt'
    ...
    if request.method == 'POST':
        os.remove('post_exercise_ecg.txt') #delete the ecg files
        os.remove('pre_exercise_ecg.txt')
        os.remove('Peaks.jpeg')
        return redirect(f'/') #go to homepage
    else: 
        with open('sampleform.csv', 'r') as sampleform:
            row=[] #intialize lists
            fields=[]
